...
- **`ImplicitTypeConversionAnalyzer` detects integer-bound DQL parameters on text columns.** DQL infers a parameter's binding type from its PHP value, so `->setParameter('code', 123)` on a `string` column binds an integer, and MySQL and MariaDB then read every row instead of looking the value up in the index (checked with `EXPLAIN` for emulated and native prepares). `QueryData` now keeps the binding types recorded by Symfony's `DoctrineDataCollector` (as the `ParameterType` case name, since `json_encode()` rejects the pure enum in the profiler export), and the analyzer maps each positional `?` to its type. `findBy()`, which binds the mapped column type, is not reported. An integration test runs the queries through Symfony's debug middleware and data collector, as the profiler does.
- **`MySQLCollationAnalyzer` now reports view columns incompatible with the connection collation.** The analyzer covered the database default, base tables and foreign key columns, but its table query filters on `TABLE_TYPE = 'BASE TABLE'`, so views were never examined — a schema whose tables are uniformly correct reported clean while a view column silently held a different collation. A `CREATE VIEW` statement freezes the session collation into the literals of its definition, so a view created from a connection using `utf8mb4_general_ci` bakes that collation into every `IF(...)`, `CONCAT(...)` and `IFNULL(...)` result column it defines. When the application later compares such a column against a literal of its own connection collation, both operands carry the same coercibility, MySQL has no rule to arbitrate between them, and the query fails outright with error 1267 (_Illegal mix of collations_) rather than degrading. The new check joins `information_schema.COLUMNS` against `information_schema.VIEWS` and reports columns whose collation differs from `@@collation_connection`, at critical severity, naming each view and column and pointing at the two available fixes: recreating the view from a connection using the application collation, or pinning the literals with an explicit `COLLATE`, which raises their coercibility and makes the definition independent of the session that created it.
...
- **Platform tests against real MySQL and PostgreSQL servers.** The platform-specific Configuration analyzers and `OverprivilegedDatabaseUserAnalyzer` only ran against mocks or SQLite, and the MySQL/PostgreSQL connection helpers in `PlatformAnalyzerTestHelper` were unused. `tests/Integration/Platform` now runs every one of them on both servers, and checks their detections against conditions the tests set up themselves (a MyISAM table, a `utf8mb3` table, a table collation, the session `sql_mode`, `statement_timeout` and `idle_in_transaction_session_timeout`). The tests are skipped when no server is reachable; a new `Platform Tests` CI job provides MySQL 8.4 and PostgreSQL 16 and fails if they are skipped. They were run locally against MariaDB 10.11 and PostgreSQL 16.

### Changed

- **`ImplicitTypeConversionAnalyzer` now reports the conversion that actually disables an index.** It guessed column types from their names and reported numeric columns compared to quoted numbers (`user_id = '42'`) and date columns compared to integers. `EXPLAIN` on MariaDB and PostgreSQL shows the index still used in those cases: the literal is converted once. The comparison that loses the index is the reverse, a text column compared to a number (`code = 123`), which MySQL and MariaDB evaluate by casting every row, while PostgreSQL rejects it. The analyzer now reads column types from the Doctrine metadata and reports only that case; it stays silent without an entity manager or for columns Doctrine does not map. A MySQL platform test runs `EXPLAIN` on each case against a real server.
- **`EntityManagerClearAnalyzer` now warns from 50 writes on a table instead of 20.** Twenty writes in one request, such as an order saved with twenty lines in a single `flush()`, hold twenty entities in the identity map and carry no memory risk, yet were reported as a "Memory Leak Risk" warning. The default follows the value `config/packages/doctrine_doctor_recommended.yaml` already described as realistic for batch processing; `analyzers.entity_manager_clear.batch_size_threshold` still overrides it.
- **PHPStan now checks argument types.** The `argument.type` and `argument.templateType` identifiers were ignored project-wide, which hid the `SqlStructureExtractor` crash fixed above. The remaining findings were imprecise annotations: a stale `@phpstan-return` that overrode the correct `@return` in `QueryCachingOpportunityAnalyzer`, list/array and `class-string` mismatches, and eight suggestion templates that passed an unchecked context value to `basename()`. With those fixed, both identifiers are enforced again.
- **Array access on association and embedded mappings removed ahead of ORM 4.** ORM 4 drops `ArrayAccess` from its mapping objects, but ORM 3 only deprecates it on some of them and not at all on `AssociationMapping`, so `$associationMapping['targetEntity']` works silently today and becomes a fatal error on ORM 4. Thirteen analyzers and helpers now read `AssociationMapping` and `EmbeddedClassMapping` through their properties and `type()`. A custom PHPStan rule, `NoMappingArrayAccessRule`, reports any array access on a Doctrine mapping object so the pattern cannot come back.
- **`MissingTransactionOnBatchAnalyzer`, `NPlusOneSqlAnalyzer`, `TypeHintMismatchAnalyzer` and `MissingTransactionOnBatchIssue` are no longer `final`**, like every other analyzer and issue, so they can be extended or decorated the same way.
...
- **`FlushInLoopAnalyzerModern`**, a draft variant of `FlushInLoopAnalyzer` that was registered but never tagged, so it never ran. Its real-database scenarios now exercise `FlushInLoopAnalyzer`.

### Fixed

- **`MissingIndexAnalyzer` reported missing indexes on PostgreSQL for plans that used one.** Its PostgreSQL plan reader never filled `key` or `possible_keys`, so an `Index Scan` or bitmap scan estimating at least 1000 rows, or a `Seq Scan` PostgreSQL chose over an existing index for low selectivity, was reported with a `CREATE INDEX` suggestion. Plan nodes now carry the index they use (a bitmap heap scan takes it from its bitmap index scan child), and a sequential scan whose `Filter` starts an existing index lists that index as a candidate, like MySQL's `possible_keys`. A PostgreSQL platform test runs both cases, plus a filter on an unindexed column that is still reported, against a real server.
- **`MissingIndexAnalyzer` suggested creating indexes that already existed.** It reported "Missing Index Detected" with a `CREATE INDEX` suggestion whenever `EXPLAIN` estimated at least 1000 rows, even when the plan used an index (`ref`, `range`, full index scan) or listed one in `possible_keys` that the optimizer skipped for low selectivity. On MariaDB, a year-wide range on an indexed `created_at` column was reported this way. A missing index is now reported only when `EXPLAIN` shows no candidate index at all; a MySQL platform test runs both cases against a real server.
- **`NestedRelationshipN1Analyzer` missed the nested N+1 a loop actually produces.** It required each level's queries to form a contiguous block, but a `foreach` calling `$comment->getPost()->getAuthor()` makes Doctrine load the levels in turn: post, author, next post, next author. On real Doctrine SQL the analyzer therefore never reported the classic case. A non-contiguous group now counts when it loads rows by primary key (lazy to-one proxies) and only alternates with other such groups, with no unrelated `SELECT` in between; alternating loads by foreign key, which are per-row collection loads, stay excluded. A new integration test runs the loop on a real `EntityManager` and failed before this fix.
- **`FindAllAnalyzer` reported filtered queries as unrestricted.** It inferred the presence of a `WHERE` clause from the bare columns found in it, so a query filtering only on a function of a column, such as the case-insensitive lookup `WHERE LOWER(email) = ?`, was reported as selecting a whole table. It now asks the SQL parser whether the statement has a `WHERE` clause at all.
- **`SqlStructureExtractor` crashed with a custom join extractor.** When given a `JoinExtractorInterface` other than `SqlJoinExtractor`, it built its default pattern detector with `null`, which the detector's constructor rejects with a `TypeError`. It now falls back to a default `SqlJoinExtractor` for the detector. Found by re-enabling PHPStan's `argument.type` check.
- **Required to-one associations were never detected.** `EntityStateConsistencyAnalyzer` (required association without cascade persist) and `PropertyTypeMismatchAnalyzer` (nullable property on a `NOT NULL` join column) read the join column as an array, but on ORM 3 it is a `JoinColumnMapping` object, so every association was treated as nullable and neither check could ever fire. They now use `MappingHelper::isJoinColumnNullable()`, and fixtures cover both detections.
- **`CascadeAllAnalyzer` under-rated `cascade="all"` on ManyToMany and OneToOne associations.** It guessed the association type from the mapping class name, looking for `ManyToManyAssociation` and `OneToOneAssociation`, but ORM 3 instantiates `ManyToManyOwningSideMapping`, `OneToOneOwningSideMapping` and their inverse-side counterparts. Such associations were reported as `Unknown`, so a ManyToMany cascading everything to an independent entity came out as a warning instead of critical. The test covering it accepted either severity, which hid the problem; it now asserts critical. The same class-name guessing was duplicated in eight analyzers and helpers; they now share `MappingHelper::getAssociationType()`, which reads `AssociationMapping::type()`.
- **`FlushInLoopAnalyzer` missed flush-in-loop in real applications.** It only recognised a flush when a write was immediately followed by a `SELECT`. But `flush()` wraps its writes in a transaction, and Symfony's Doctrine debug middleware logs `"START TRANSACTION"` and `"COMMIT"` as queries, so in a profiled request the write is followed by `COMMIT` and the pattern never matched; the canonical `persist()` + `flush()` loop, with no read in between, was not recognised either. A `COMMIT` closing at least one write now ends a flush. The only test running the analyzer against a real database targeted the unused draft variant; it now covers the active analyzer, and failed before this fix.
...
- **`DoctrineCacheAnalyzer` reported proxy auto-generation that Doctrine ignores.** With native lazy objects (ORM 3.4+ on PHP 8.4, always on since DoctrineBundle 3.1) Doctrine no longer generates proxy classes, so `auto_generate_proxy_classes` has no effect, and `Configuration::getAutoGenerateProxyClasses()` is deprecated (doctrine/orm#12005). The runtime check now skips when native lazy objects are enabled, instead of raising a critical finding and a deprecation on every profiled request.
- **The partial object advice could turn a full load into an N+1.** `PartialObjectAnalyzer` recommended `SELECT PARTIAL` first and described the result as read-only. With native lazy objects, reading a field left out of the `PARTIAL` list loads it on access, one query per entity, and partial objects stay managed rather than read-only. The suggestion now leads with DTO (`SELECT NEW`), array and scalar hydration, keeps partial objects last, and states the lazy-loading cost; the `excessive_hydration` template carries the same warning.
- **Pagination advice pointed to a deprecated class.** ORM 3.7 deprecates `Doctrine\ORM\Tools\Pagination\Paginator` in favour of `OffsetPaginator`, and adds a `CursorPaginator` for keyset pagination. When they are installed, `SetMaxResultsWithCollectionJoinAnalyzer` now recommends `OffsetPaginator` with a `Window`, and `DeepOffsetPaginationAnalyzer` points to `CursorPaginator`; older ORM versions keep the previous advice.
- **Queries issued by the ORM 3.7 paginators were not recognised as paginator queries.** `PaginatorQueryDetector` only matched `Pagination\Paginator`, so `JoinOptimizationAnalyzer` and `UnusedEagerLoadAnalyzer` would report the internal queries of `OffsetPaginator` and `CursorPaginator` as if the application had written them. Any class of the `Doctrine\ORM\Tools\Pagination` namespace now counts.

## [2.10.3] - 2026-09-09

### Added

- **Three new Integrity analyzers for mappings Doctrine accepts but ORM 4 will reject.** They cover mappings that Doctrine itself does not reject, drawn from the ORM 3.6 and DBAL 4.4 deprecation cycle. `DiscriminatorColumnAnalyzer` reports a discriminator column whose length cannot hold the longest key of its own `DiscriminatorMap` — the value is truncated on write and the row can no longer be resolved back to its class — and, separately, a Single Table Inheritance hierarchy with no index on the discriminator column, where every subclass query appends a filter on it and scans the whole table. `NullablePrimaryKeyAnalyzer` reports primary key columns mapped as `nullable`, deprecated in ORM 3.6 (doctrine/orm#12126) and rejected in 4.0. `StringDefaultExpressionAnalyzer` reports raw SQL string defaults on temporal columns, deprecated in ORM 3.6 (doctrine/orm#12252) in favour of the `DefaultExpression` value objects added in DBAL 4.4 (doctrine/dbal#7195); it mirrors `SchemaTool` exactly, matching only temporal types whose default equals the current platform's own current-date/time SQL, so a string column holding the same literal is left alone.
- **A fourth check was considered and deliberately left out.** Detection of an entity missing from its root's `DiscriminatorMap` cannot work: `ClassMetadataFactory` throws `mappedClassNotPartOfDiscriminatorMap` on that mapping, so `getAllMetadata()` never returns and no analyzer can observe it. Abstract classes escape the throw but legitimately carry no discriminator value. The same applies to invalid enum entries, which `invalidEntriesInDiscriminatorMap` already rejects.
...
- **Rector now runs in CI.** Rector was in `composer check` but in none of the workflow jobs, which is how 106 files accumulated without a single red build. The gate is now enforced where it is visible.

### Changed

- **The three query field accessors every SQL-inspecting analyzer needs are now a shared trait.** `extractSQL()`, `extractBacktrace()` and `extractExecutionTime()` were copied into each one, byte-identical in all but one file, and are now a single `QueryFieldAccessorTrait` under `src/Analyzer/Concern/`. 13 analyzers drop 224 lines net. The accessors exist because a collected query reaches an analyzer either as a `QueryData` object or as the legacy array shape, so each one has to handle both. `QueryCachingOpportunityAnalyzer` held a variant written with `if`/`return` instead of a ternary, and one without a `?? ''` fallback on an already-typed property; both are equivalent and were folded into the shared version. The public `AnalyzerInterface` is untouched — passing a prepared context object to `analyze()` would have been the deeper fix, but that contract is documented for third-party analyzers and changing it would break them.
- **Four redundant top-level directories are folded away.** This takes the tree from 20 to 16 and the Deptrac layer list from 18 to 15. `Helper/` held a single class while `Analyzer/Helper/` held nine, so `MappingHelper` moves there and the redundant layer disappears; `Cache/SqlNormalizationCache` moves under `Infrastructure/Cache/`, where an infrastructure-level static cache belongs. Both had their namespaces and 39 imports updated, and the `StaticAccess` exception list in `phpmd.xml` was pointing at the old fully-qualified names, which is what caught the move. `Utils/` (68 references) and `Metadata/` are deliberately left alone: renaming them costs more than the tidiness is worth, and `Metadata/` carries the EntityManager decorator, a real boundary. Deptrac stays at 0 violations with no baseline, and the uncovered count drops from 1810 to 1805 — exactly the five deleted classes.
- **`doctrine/orm` in `require-dev` is pinned to `^3.0`, no longer resolving the unreleased 4.x branch.** No 4.x has been released — Packagist lists only the `4.0.x-dev` branch — so the upper bound allowed the development branch of an unreleased major to be resolved, whose BC breaks are by definition unknown and untested here. ORM 3.6 already deprecates mappings that 4.0 will reject, which is exactly the window in which an accidental resolution would fail in confusing ways. The constraint was widened in ec60e80 as part of a broad modernization commit rather than as a deliberate targeting of ORM 4, and nothing in the CI matrix, the documentation or the code refers to it. Runtime dependencies are unaffected: the bundle requires `doctrine/doctrine-bundle` rather than the ORM directly, so this changes only what the bundle's own test suite resolves against.
- **Rector's PHP 8.4 modernization is applied across 106 files.** `composer check` was red on `main` even on an untouched checkout, because Rector reported those files while none of the CI jobs ran it — so the combined gate had drifted out of reach locally without anything failing publicly. By volume the rules were `NullToStrictStringFuncCallArgRector` (43 files), `AddOverrideAttributeToOverriddenMethodsRector` (26), `NewMethodCallWithoutParenthesesRector` (12), `StringClassNameToClassConstantRector` (10), and fourteen others with five or fewer occurrences. Two needed correcting afterwards: `NewInInitializerRector` promoted four lazily-initialized dependencies into constructor defaults but kept the parameter nullable, turning four non-nullable properties into nullable ones — PHPStan caught it as six `cannot call method on X|null` errors, and the nullable markers were removed since the default is always an instance; ECS then reformatted the constructors Rector had collapsed onto single lines. The suggestion templates were validated separately, since that directory is excluded from ECS, PHPStan, Deptrac and PHPMD and is policed only by `bin/validate-suggestion-templates.php` — all 112 pass, which matters because `NullToStrictStringFuncCallArgRector` inserts casts of exactly the shape that validator rejects when they are not a known escaper. Deprecation notices in the test suite dropped from 10 to 8.

### Fixed

- **The analyzer catalog documented 58 of the 98 registered analyzers, with stale category totals.** It announced 19 Performance, 4 Security and 35 Integrity against a reality of 30, 6 and 53, with Configuration carrying no total at all. All 37 missing analyzers are now listed, the totals match the code, and Configuration gained an overview section. Three trait analyzers (`Timestampable`, `Blameable`, `SoftDeleteable`) were described under Configuration although they live in `Integrity/`, and `GetReferenceAnalyzer` was listed under Integrity although it lives in `Performance/` — both are now filed under the category they actually belong to. Descriptions were read from each analyzer's source rather than scraped, since two of them carry no docblock and the titles are built with `sprintf()`. Two classes stay out of the catalog on purpose and the page now says why: `MissingIndexAnalyzerConfig` is a configuration object, and `FlushInLoopAnalyzerModern` is excluded from the `doctrine_doctor.analyzer` tag, so it never runs. Also fixes two duplicate section numbers (`3.2`, `5.2.2`) that predate this change.
- **Two analyzers reporting different problems on the same table no longer swallow each other.** `IssueDeduplicator::getTableRelatedSignature()` had grouped issues on the text of their title: any issue whose title contained `Index` was filed under `table_performance:{table}`, and `ORDER BY` or `findAll` under `table_query:{table}`. Two analyzers reporting genuinely different problems on the same table were merged whenever both titles happened to mention an index, and the second one vanished from the panel — reproduced with `missing_index` and `unique_entity_without_index` on a `users` table. The method now keys on `IssueType` (`MISSING_INDEX`, `FIND_ALL`), with the title match kept only for issues carrying a type that is not a recognised `IssueType` value, since some are still created with a literal string rather than the enum. Three characterisation tests cover the collision, the proxy pair that must stay separate, and the same-type case that must still merge.
- **The runtime proxy auto-generation finding no longer disappears behind the config-file one.** The two analyzers reported proxy auto-generation under the identical title `Proxy Auto-Generation Enabled in Production`, at critical severity. Identical titles gave them the same deduplication signature, so `IssueDeduplicator` collapsed them into one and filed the runtime finding under `getDuplicatedIssues()`, where the panel never shows it — the config-file diagnosis was displayed and the independent runtime confirmation silently disappeared. The titles now name their source — `(config file)` and `(runtime)` — and the runtime message states that the value comes from the EntityManager and therefore includes overrides applied after the configuration files are loaded. Detection is unchanged: the two were kept separate rather than merged because they read different sources and neither covers the other. `AutoGenerateProxyClassesAnalyzer` parses the production YAML and has no environment guard, so it warns from the dev profiler before anything is deployed; `DoctrineCacheAnalyzer` reads `getAutoGenerateProxyClasses()` behind a `prod` guard and is the only one that sees a runtime override the YAML does not show. Each of the two divergent cases is detected by exactly one of them, verified across the full matrix of config value, runtime value and environment.

- **Development files are no longer shipped inside the Composer package.** A `.gitattributes` now marks the CI workflows, the quality tooling configuration, `docs/` and `tests/` as `export-ignore`, so `composer require` no longer pulls them into `vendor/`. `.castor.stub.php` is deleted outright: it carries a `You can safely ignore this file in your VCS` notice, and IDEs were loading it from `vendor/` in consuming projects and reporting its class definitions as duplicates. Reported and fixed by @thePanz in #137.

### Removed

- **`src/Detector/` and `src/Generator/` are deleted, 202 lines never wired to anything.** `DetectorInterface` / `PatternDetector` were never imported anywhere in the repository's entire git history, and the detectors actually in use live in `src/Analyzer/Helper/` and `src/Analyzer/Parser/`. `MigrationGenerator` and `RepositoryCodeGenerator` had no PHP usage either: their only two references were service registrations in `config/services.yaml`, and since `_defaults` sets `public: false`, Symfony strips them at compile time — they were unreachable from the container and undocumented, so no consumer could depend on them. Both directories date from 2025-11-10 and were only ever touched by repo-wide refactors.

...
- **`BulkInsertAnalyzer` recommends DBAL for imports that insert rows one by one.** The ORM writes one `INSERT` per persisted entity, so an import of hundreds of rows costs hundreds of round trips. When at least `bulk_insert.threshold` (default 100) single-row `INSERT` statements target the same table, the analyzer suggests a chunked multi-row `INSERT` through `Connection::executeStatement()`. For a mapped entity it lists what DBAL would bypass, read from the metadata: lifecycle callbacks, entity listeners, and identifiers generated by `IDENTITY` or sequences. Statements that already insert several rows, or `INSERT ... SELECT`, are not reported. An integration test runs it on the SQL of a real `flush()` of 150 entities and on the same rows written with DBAL batches.
...
- **`BulkOperationAnalyzer` and `DTOHydrationAnalyzer` suggestions offer DBAL where it fits.** `BulkOperationAnalyzer` told users to replace repeated `UPDATE`/`DELETE` statements with a bulk operation, but its template only showed `flush()` and `clear()`, which still sends one statement per entity. The suggestion now leads with one set-based statement matching the detected kind, in DQL or through `Connection::executeStatement()`, states what it bypasses (callbacks, listeners, cascades, stale loaded entities), and keeps flush and clear in batches as the ORM option. Its title no longer speaks of a memory leak. The DTO hydration suggestion adds plain rows through `fetchAllAssociative()` for reports that only read values.
...
- **Pagination checks no longer mistake SQL text for ordering.** `ORDER BY` inside a literal, comment or unrelated subquery does not suppress the warning. The Doctrine paginator exception now checks the actual ordering of its `dctrn_result` derived table, and the single-row exception reads the outer query's limit.
- **PostgreSQL index candidates must match a bare-column comparison.** A column name appearing in a string value or inside a function or cast no longer makes its ordinary index suppress a missing-index warning. Partial indexes and filters containing alternative `OR` branches are not inferred to be usable from the column name alone.

- **The second-level cache opportunity stayed next to the N+1 it duplicates.** With bound parameters recorded, as the Symfony profiler does, a lazy-loading loop also raised `Doctrine 2LC Opportunity (10 fast entity loads)`, which the deduplicator did not merge into the N+1: its title pattern missed "10 fast entity loads", its entity pattern read `loads` from "entity loads" because `/i` made `[A-Z]` match lowercase, and a description mentioning "benefit from Doctrine" was read as table `Doctrine` before the SQL. The title pattern now accepts up to two words before the unit and `loads`, the entity pattern only matches capitalised names, and the SQL `FROM` table is read before description prose. The integration test query logger now records bound parameters and types like Symfony's profiler, so integration tests see what production sees; that is how this surfaced.
- **`DQLInjectionAnalyzer` reported one DQL literal twice.** A query such as `WHERE u.email = 'user1@example.com'` matched both the unparameterized-literal check and the generic pattern scan, which produced `DQL Injection Risk: ... concatenated literal value` and `Security Warning: 1 queries with potential injection risks` for the same statement. Each query now yields one issue: the critical pattern issue when the SQL shows a tautology or similar, otherwise the unparameterized-literal one. Integration tests cover both through real DQL.
- **`NPlusOneAnalyzer` reported the same query repeated as an N+1.** Five identical `SELECT COUNT(...)` calls were reported as `N+1 Query Detected (unknown)` with fetch-join advice, though nothing varies between runs and no join can help. The repeated query analyzers already cover that case with caching advice. A group whose statements are identical, without placeholders or with the same bound parameters, is now left to them; a group with placeholders but no recorded parameters is still reported.
- **`TransactionBoundaryAnalyzer` reported the recommended `wrapInTransaction()` pattern as a critical nested transaction.** DBAL 4 nests transactions with savepoints, so a `flush()` inside `wrapInTransaction()` runs `SAVEPOINT DOCTRINE_2`, which MySQL, MariaDB and PostgreSQL honour. The analyzer counted the savepoint as a nested transaction and claimed inner transactions are ignored. Savepoints now only track nesting; the issue is raised for a second `START TRANSACTION` on an open transaction, and its description now says what actually happens: MySQL and MariaDB commit the open transaction implicitly. Savepoints also broke the other checks: each flush in an explicit transaction opened its own savepoint, so `Multiple Flush in Transaction` counted one flush per savepoint and never fired on DBAL 4, and each savepoint reset the transaction start time used by the long-transaction check. Savepoints now share the flush count and start time of their enclosing transaction. Two existing integration tests, skipped because "SQLite emits SAVEPOINT" (DBAL 4 does on every platform), run again, and a new one covers `wrapInTransaction()` with a real flush.
- **`PaginationWithoutOrderByAnalyzer` asked for an ORDER BY that Doctrine's Paginator had already applied.** The Paginator moves the DQL `ORDER BY` into a derived table and puts the `LIMIT` on the outer query, so a correctly ordered paginated query was reported as non-deterministic, with advice to add the clause it already had. An `ORDER BY` in a subquery now counts; a Paginator query without any `ORDER BY` is still reported. An integration test runs both through the real Paginator.
- **One N+1 loop was reported up to three times.** `IssueDeduplicator` merges the issues several analyzers raise for one repeated query by table and count, but it read the table from issue text with patterns lacking word boundaries: "execution time" yielded the table `time`, "Collection N+1" the table `n`, "Pagination Without" the table `without`. On Doctrine's SQL, a lazy to-one loop therefore kept `N+1 Query Detected`, `Lazy Loading in Loop` and `Lazy Loading Detected`, and a lazy collection loop kept `N+1 Query Detected` and `Frequent Query Executed`. Each loop now yields the single N+1 issue, with the others attached as duplicates. An integration test runs the real analyzers on both loops.
- **`NPlusOneSqlAnalyzer` reported ORM lazy loading as an N+1 loop "in DBAL code".** It grouped every repeated `SELECT`, so the classic lazy-loading loop produced two issues: `NPlusOneAnalyzer`'s, and this one, which blamed DBAL code and suggested hand-written SQL where a fetch join is the fix. Both survived deduplication. Queries Doctrine generated, recognised by its `<column>_<n>` column aliases or a `Doctrine\ORM` backtrace frame, are now left to `NPlusOneAnalyzer`. For real DBAL loops the suggestion now shows the repeated query rewritten with `IN (?)` and an `ArrayParameterType` parameter.
- **SQL injection suggestions showed code from the wrong layer or an API DBAL 4 rejects.** The template used by `QueryBuilderBestPracticesAnalyzer`, which detects concatenation in an ORM QueryBuilder, showed raw DBAL code and fixed it with `bindValue(1, $id, \PDO::PARAM_INT)`, which DBAL 4 rejects because it expects a `ParameterType`. The fix now matches the layer: `setParameter()` for a QueryBuilder, and `executeQuery()` with `ParameterType` for raw SQL. The raw-query suggestion also no longer calls an `(int)` cast unsafe.
